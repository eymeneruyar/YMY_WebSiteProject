...
import YMY.repositories.DispatchNoteRepository;
...
public class DispatchNoteDto {
    ...
    final DispatchNoteRepository dispatchNoteRepository;
    final WorksRepository worksRepository;
    final UserService userService;
    final CacheService cacheService;
    public DispatchNoteDto(CompanyRepository companyRepository, CustomerRepository customerRepository, DispatchNoteRepository dispatchNoteRepository, WorksRepository worksRepository, UserService userService, CacheService cacheService) {
    ...
    public Map<Check,Object> listCompanyByUserId(){
        ...
        try {
            if(user.getId() != null){
                ...
                Util.logger(error, DispatchNote.class);
            }
        } catch (Exception e) {
            String error = "Firmalar getirilirken bir hata oluştu!";
            hm.put(Check.status,false);
            hm.put(Check.message,error);
            Util.logger(error + " " + e, DispatchNote.class);
        }
        return hm;
    }
